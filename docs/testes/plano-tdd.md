...
| RN-E03 | `RnE03EmissaoEmLotePlanilhaTest` | `rnE03_cf_planilhaComOitoPassageirosValidosProcessaTodosComSugestaoDentroDaPolitica` | GREEN — discrepância de fixture identificada (massa de teste continha o literal `"...8 linhas válidas..."` em vez de 8 linhas CSV reais) e confirmada com o stakeholder; fixture corrigida com 8 linhas CSV válidas e distintas (autorizado explicitamente, não altera as asserções nem o resultado esperado) |
...
**Execução (`mvn test -Dtest='RnE*'`):** 21 testes — 21 passaram (GREEN), 3 `@Disabled` (SKIPPED). **Atualizado em 2026-09-18 (fase Green)**: lógica de negócio implementada em `b2b/*`; fixture de `RnE03EmissaoEmLotePlanilhaTest#rnE03_cf_planilhaComOitoPassageirosValidosProcessaTodosComSugestaoDentroDaPolitica` corrigida mediante autorização explícita do stakeholder (discrepância de dados de teste, não de regra de negócio).
...
| RN | Classe de teste | Método (caso) | Status |
|---|---|---|---|
| RN-G04 | `RnG04ConciliacaoFinanceiraIdempotenteTest` | `rnG04_cf_conciliacaoDeUmPeriodoFechadoGeraRelatorioConsolidadoSemDuplicidade` | GREEN — `FinancialReconciliationService.reconciliar` gera um `Lancamento` por evento, chave de idempotência `periodo:eventoId` |
| RN-G04 | `RnG04ConciliacaoFinanceiraIdempotenteTest` | `rnG04_conf_reprocessamentoDoMesmoPeriodoNaoGeraDuplicidadeViaIdempotencia` | GREEN — reprocessamento reutiliza `Map` por chave de idempotência (dedupe), quantidade de lançamentos inalterada |
| RN-G04 | `RnG04ConciliacaoFinanceiraIdempotenteTest` | `rnG04_proib_lancamentoDuplicadoAposReprocessamentoNuncaPodeOcorrer` | GREEN — chaves de idempotência permanecem únicas após reexecução |
| RN-G04 | `RnG04ConciliacaoFinanceiraIdempotenteTest` | `rnG04_cf_trilhaDeAuditoriaDeConciliacaoERegistradaDeFormaImutavelERetidaPorNoMinimoCincoAnos` | GREEN — `trilhaDeAuditoria` retorna `AuditEntry` com `imutavel=true` e `retencaoAnosMinima=5` |
...
| RN-G03 | `RnG03AlertasAutomaticosDePadroesAnomalosTest` | `rnG03_cf_volumeDeReembolsosAtingeTresVezesAMediaMovelDisparaAlertaDentroDoSla` | GREEN — `RefundAnomalyDetectionService.avaliarAnomalia` dispara quando `volume >= 3 × mediaMovel` |
| RN-G03 | `RnG03AlertasAutomaticosDePadroesAnomalosTest` | `rnG03_lim_volumeExatamenteIgualATresVezesAMediaDeveDispararNoLimiar` | GREEN — limiar `>=` inclui o valor exato de 3× |
| RN-G03 | `RnG03AlertasAutomaticosDePadroesAnomalosTest` | `rnG03_inv_volumeLigeiramenteAbaixoDoLimiarNaoDeveDispararAlerta` | GREEN — 2,9× não atinge o limiar, `Optional.empty()` |
| RN-G03 | `RnG03AlertasAutomaticosDePadroesAnomalosTest` | `rnG03_lim_alertaDisparadoExatamenteAosQuinzeMinutosAindaDentroDoSla` | GREEN — `dentroDoSlaDeQuinzeMinutos` usa `<=` 15 minutos |
| RN-G05 | `RnG05MediacaoDeDisputasDeReembolsoComSlaTest` | `rnG05_cf_aberturaDeDisputaEntrePassageiroECompanhiaIniciaFluxoDeMediacaoComAcompanhamentoDeSla` | GREEN — `RefundDisputeMediationService.abrirDisputa` retorna status `ABERTA` com `slaAcompanhado=true` |
| RN-G05 | `RnG05MediacaoDeDisputasDeReembolsoComSlaTest` | `rnG05_lim_slaDeMediacaoUsaBaselineDeQuinzeDiasCorridosConfirmadoPeloStakeholder` | GREEN — `slaMaximo` fixado em `Duration.ofDays(15)`, baseline do stakeholder (Seção 3.4, item 5) |
| RN-G01 | `RnG01OnboardingPadronizadoDeCompanhiasParceirasTest` | `rnG01_cf_novaCompanhiaCompletaDocumentacaoETestesTecnicosConcluiOnboardingDentroDoBaseline` | GREEN — `PartnerAirlineOnboardingService.concluirOnboarding` conclui em 10 dias úteis quando documentação e testes técnicos estão completos |
...
| RN-G02 | `RnG02PainelUnicoDeIndicadoresDeSaudeDaOperacaoTest` | `rnG02_cf_consultaConsolidadaDeFraudeDisputasESlaPorCompanhiaExibeIndicadoresUnificados` | GREEN — `OperationalHealthDashboardService.indicadoresConsolidados` retorna `HealthIndicators` para a companhia solicitada |
| RN-G02 | `RnG02PainelUnicoDeIndicadoresDeSaudeDaOperacaoTest` | `rnG02_proib_vazamentoDeDadoDeUmaCompanhiaParaOPainelDeOutraNuncaPodeOcorrer` | GREEN — `indicadoresNoContextoDe` sempre resolve os indicadores da companhia solicitante, nunca do alvo |

**Execução (`mvn test -Dtest='RnG*'`):** 15 testes — 13 GREEN, 0 falharam/erraram, 2 `@Disabled` (SKIPPED). **Atualizado em 2026-09-17 (fase GREEN)**: `PartnerAirlineOnboardingService`, `OperationalHealthDashboardService`, `RefundAnomalyDetectionService`, `FinancialReconciliationService` e `RefundDisputeMediationService` implementados. O `@Disabled` de RN-G04 foi mantido, pois a Decisão do item 7 resolveu RN-H02, mas cria um conflito NOVO e não mitigado com RNF-23 que exige escalada formal — não é resolução silenciosa (ver relatório de implementação).

### Grupo H — Segurança e conformidade (tokenização, LGPD, RBAC cross-tenant, OAuth2/auditoria de API)

| RN | Classe de teste | Método (caso) | Status |
|---|---|---|---|
| RN-H01 | `RnH01TokenizacaoPagamentoTest` | `rnH01_cf_cartaoSalvoParaComprasFuturasEArmazenadoComoTokenNuncaComoPan` | GREEN — `PaymentCardVaultService.tokenize` gera token opaco (`tok_<uuid>`), nunca o PAN |
| RN-H01 | `RnH01TokenizacaoPagamentoTest` | `rnH01_proib_nenhumArmazenamentoPersistentePodeConterPanEmTextoClaro` | GREEN — `representacaoPersistida` só referencia o token, nunca o PAN em texto claro |
| RN-H02 | `RnH02ExclusaoLgpdTest` | `rnH02_cf_titularSemTransacoesPendentesDeAuditoriaEExcluidoEmAte15DiasCorridos` | GREEN — `PersonalDataErasureService.solicitarExclusao` retorna `EXCLUIDO` com prazo `<=15` dias |
| RN-H02 | `RnH02ExclusaoLgpdTest` | `rnH02_conf_titularComTransacaoEmRetencaoDeAuditoriaTemExclusaoTotalDentroDoPrazo` | GREEN — **decisão do stakeholder (Seção 3.4, item 7)**: exclusão prevalece sobre retenção de auditoria, `trilhaFinanceiraExcluida=true`, dentro de 15 dias. **Conflito ABERTO e ESCALADO, não resolvido silenciosamente**: esta implementação segue literalmente a decisão do stakeholder e o teste, mas contradiz diretamente RNF-23/RF-32 (retenção mínima de 5 anos da trilha de auditoria/financeira), implementadas em `FinancialReconciliationService`/RN-G04 no mesmo módulo. Nenhuma tentativa de reconciliação em código foi feita (ADR-09/pseudonimização rejeitado) — ver relatório de implementação para o registro formal do risco |
| RN-H02 | `RnH02ExclusaoLgpdTest` | `rnH02_cf_dadoDeMenorDeIdadeSujeitoAExclusaoRecebeMesmoTratamentoDeRnf25` | GREEN — mesmo tratamento de RNF-25, sem diferenciação por menoridade (fonte não define regra específica) |
| RN-H03 | `RnH03RbacCrossTenantTest` | `rnH03_cf_usuarioComPapelDefinidoAcessaApenasRecursosDoSeuEscopo` | GREEN — `TenantAccessControlService.acessarRecurso` retorna o recurso quando `tenantSolicitante == tenantDono` |
| RN-H03 | `RnH03RbacCrossTenantTest` | `rnH03_proib_companhiaAAcessaDadosDaCompanhiaBEBloqueado` | GREEN — tentativa cross-tenant lança `CrossTenantAccessDeniedException` |
...
| RN-H04 | `RnH04OAuthExpiracaoTokenTest` | `rnH04_cf_logDeAuditoriaDeChamadaDeApiERegistradoERetidoPorAoMenos5Anos` | GREEN — `ApiCallAuditTrail.registrarChamadaAutenticada` retorna `AuditRecord` com retenção `>=5×365` dias |
| RN-H04 | `RnH04OAuthExpiracaoTokenTest` | `rnH04_proib_tokenDeAcessoValidoPorMaisDeUmaHoraEProibido` | GREEN — `ApiAuthService.issueToken` emite token com validade fixa de 1 hora |

**Execução (`mvn test -Dtest='RnH*'`):** 10 testes — 9 GREEN, 0 falharam/erraram, 1 `@Disabled` (SKIPPED). **Atualizado em 2026-09-17 (fase GREEN)**: `PaymentCardVaultService`, `PersonalDataErasureService`, `TenantAccessControlService`, `ApiAuthService` e `ApiCallAuditTrail` implementados. **Risco aberto reafirmado**: RN-H02 (exclusão total, inclusive trilha financeira) implementada exatamente como decidido pelo stakeholder (Seção 3.4, item 7) e exigido pelo teste, mas isso conflita diretamente com RNF-23/RF-32 (retenção ≥5 anos, RN-G04). Este é um conflito deliberado e já documentado como não mitigado — requer escalada formal a quem detém autoridade sobre RNF-23 antes de produção; não foi "corrigido" em código.
...
| RN | Classe de teste | Método (caso) | Status |
|---|---|---|---|
| RN-J01 | `RnJ01ResponsividadeTest` | `rnJ01_cf_renderizacaoEmViewportMinimoDe360PxEFuncionalComLcpAte2_5s` | GREEN — `ResponsiveRenderingService.renderizar` funcional entre 360–1920px, LCP simulado fixo em 2s (`<=2,5s`) |
| RN-J01 | `RnJ01ResponsividadeTest` | `rnJ01_lim_renderizacaoEmViewportMaximoDe1920PxEFuncional` | GREEN — viewport máximo suportado (1920px) funcional |
...
| RN-J02 | `RnJ02CompatibilidadePlataformaTest` | `rnJ02_cf_usoNasDuasVersoesMaisRecentesDeIosAndroidChromeSafariEdgeTemSuporteCompleto` | GREEN — `PlatformCompatibilityService.verificarSuporte` retorna `COMPLETO` para iOS/Android/Chrome/Safari/Edge |
...
**Execução (`mvn test -Dtest='RnJ*'`):** 5 testes — 3 GREEN, 0 falharam/erraram, 2 `@Disabled` (SKIPPED). **Atualizado em 2026-09-17 (fase GREEN)**: `ResponsiveRenderingService` e `PlatformCompatibilityService` implementados com faixas/matriz simples suficientes para o contrato asserido pelos testes.
